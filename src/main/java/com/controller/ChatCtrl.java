...
public class ChatCtrl {
    ...
    @ResponseBody
    public JSONObject upimg(@RequestParam(value = "pic", required = false) MultipartFile file) throws IOException {
    ...
    @ResponseBody
    public JSONObject upfile(@RequestParam(value = "pic", required = false) MultipartFile file) throws IOException {
        JSONObject res = new JSONObject();
        JSONObject resUrl = new JSONObject();
        String filename = UUID.randomUUID().toString().replaceAll("-", "");
        String ext = FilenameUtils.getExtension(file.getOriginalFilename());//获得文件扩展名
